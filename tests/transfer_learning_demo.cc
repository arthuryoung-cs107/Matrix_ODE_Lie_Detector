...
int main()
{
  ...
  const char Amat_letter[] = "R"; const char Amat_svd_name[] = "Rmat_svd";
  ...
  double tol_use_A = 1e-3;
  {
    LD_Theta_stack Theta_stack_A(Sobs.ncrvs_tot,fspace0.ndof_full);
    Theta_stack_A.load_Theta_tns(Amat_svd.VTtns);
    ...
    for (size_t icrv = 0; icrv < Sobs.ncrvs_tot; icrv++)
    {
      ...
      printf("%d ", nulldim_i);
      nnsat_diff_A[icrv] = Theta_stack_A.nsat_theta_spcvec[icrv]-nulldim_i;
  ...
  double tol_use_L = 1e-10;
  {
    ...
    for (size_t icrv = 0; icrv < Sobs.ncrvs_tot; icrv++)
    {
      ...
      printf("%d ", rank_i);
      nnsat_diff_L[icrv] = rank_i-Theta_stack_L.nsat_theta_spcvec[icrv];
