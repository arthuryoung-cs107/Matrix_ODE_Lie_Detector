...
dop853_integrator::dop853_integrator(ode_system &ode_, dop853_settings &set_) :
    ode_integrator(ode_), dop853_settings(set_),
    w(new double[n]), ww1(new double[n]), k1(new double[n]),
...
dop853_integrator::~dop853_integrator()
{
...
void dop853_integrator::init_counters()
{
    nstep=0;
    naccpt=0;
    nrejct=0;
...
/** Solves the ODE problem over a given time interval, storing snapshots of the
 * fields at specific times.
...
int dop853_integrator::solve(double tstart,double tend,int snaps,double **ws)
{
    ...
    // Do initial function call, and use it to estimate the initial step size
    ff(t,w,k1);nff++;
    h=hinit(hmax,posneg);
    while(true) {
        ...
        // Check for the case when the integration is close to completion, and
        // if so, try stepping to the end of the interval
...
void dop853_integrator::fixed_step(double h_,bool last) {
    h=h_;
    step12(w);
    t=tph;
...
void dop853_integrator::step12(double *p1) {
    int i;
...
double dop853_integrator::error_estimation() {
...
bool dop853_integrator::detect_stiffness() {
...
void dop853_integrator::dense_output() {
    int i;
    double ydiff,bspl;
...
void dop853_integrator::dense(double *ws,double ti) {
...
double dop853_integrator::hinit(double hmax,double posneg) {
...
void dop853_integrator::set_and_solve_time(double tstart_, double tend_, int snaps_, double **wkspc_)
{
  int exit_code = solve(tstart_,tend_,snaps_,wkspc_);
