...
class cokernal_bundle
{
  ...
  public:
    ...
    inline int collapse_cokernals(LD_vspace_measure &msr_,int gen_spawn_,bool *frdc_succ_,int *nsat_ckrn_,int *ij_prs_,double *w_ckrn_,double *vvec_ckrn_,int *isat_ckrn_)
    {
      int i_i, i_j,
          gen_iprnt[nred_succ],
          gen_jprnt[nred_succ],
      ...
      for (size_t i = 0, irdc = 0, i_ij = 0, iw = 0, iv = 0; irdc < nred_succ; irdc += (int)(frdc_succ_[i++]), i_ij+=2, iw+=vlen, iv+=vl2)
        if (frdc_succ_[i])
        {
          cokernal_space  * const ckrn_i_old = ckrn_spcs[i_i = ij_prs_[i_ij]],
                          * &ckrn_j_old = ckrn_spcs[i_j = ij_prs_[i_ij+1]];
          gen_iprnt[irdc] = ckrn_i_old->gen_spawn;
          gen_jprnt[irdc] = ckrn_j_old->gen_spawn;
          nvec_use_iprnt[irdc] = ckrn_i_old->nvec_use;
          nvec_use_jprnt[irdc] = ckrn_j_old->nvec_use;
          nV0krn_iprnt[irdc] = ckrn_i_old->n_0Vkrn;
          nV0krn_jprnt[irdc] = ckrn_j_old->n_0Vkrn;
          ...
          (ckrn_rdc[irdc] = ckrn_spcs[i_i])->init_cokernal_space(vlen,nsat_ckrn_[i],w_ckrn_+iw,vvec_ckrn_+iv,isat_ckrn_+iw); // effectively sets the underlying vector space
        }
      ...
      printf("reassigned %d cokernals:\n",nset_new);
      for (size_t i = 0; i < nset_new; i++)
      {
        cokernal_space &cki = *(ckrn_spcs[i]);
        printf("  (%d) gen=%d, n_0V=%d, iwkspc=%d, iprnt=%d, jprnt=%d, nvec=%d. i_0V: ( ",
          cki.ickrn,cki.gen_spawn,cki.n_0Vkrn,cki.iwkspc,cki.ickrn_parent,cki.jckrn_parent,cki.nvec_use);
      ...
      for (size_t irdc = 0; irdc < nred_succ; irdc++)
      {
        ...
        printf("  (irdc=%d) [iVprnt=%d, jVprnt=%d -> ickrn=%d] : gen [(%d,%d) -> %d] ; nV [(%d,%d) -> %d] ; nV0krn [(%d,%d) -> %d] \n",
          irdc,
      ...
      msr_.init_distances(Vtns_new,nV_setvec_new,nset_new);
...
class Jet_function_vector_space
{
  ...
  public:
      ...
      Jet_function_vector_space(Sobs_,fspc_,enc_,msr_)
    {
      ...
      svd0.compute_Acode_curve_svds(Acode,verbose_); svd0.print_details("svd0");
      svd0.set_Vspaces_nullspc(); rec0.print_subspace_details("rec0",false,false);
    ...
    int iterate_nullspace_cokernals(cokernal_bundle &cokern_,LD_vspace_measure &msr_,int gen_,bool verbose_=true)
    {
      ...
      k_medoids_package kmed(msr.dsym,nset);

      kSC = kmed.comp_kSC_medoids(2,verbose_);
      int nmem_clst_vec[kSC],
          imem_clst_vec[nset],
          *imem_clst_mat[kSC];

      kmed.assign_clusters(nmem_clst_vec,imem_clst_vec,imem_clst_mat,kSC,verbose_);
      ...
      nred_succ = 0;
      #pragma omp parallel reduction(+:nred_succ)
      {
        ...
        for (size_t k = 0; k < kSC; k++)
        {
          if (f_reduce_success[k] = (nmem_clst_vec[k] > 1))
          {
            ...
            kmed.comp_sort_cluster_distances(dpairs_k,ipairs_k,iipairs_k[0],imem_clst_mat[k],nmem_clst_vec[k]);
            ...
            do
            {
              ...
              Vsvd_t.decompose_U(2*vlen_full,vlen_full);
          ...
          {
            ij_nn_pairs[k][0] = ij_nn_pairs[k][1] = kmed.i_meds[k];
      ...
      printf("(Jet_function_vector_space::iterate_nullspace_cokernals) gen %d: successfully reduced %d (of kSC = %d) vector space pairs.\n Reduction details: \n",
       gen_,nred_succ, kSC);
      for (size_t k = 0; k < kSC; k++)
      {
        printf("  k=%d: imed=%d, nclst=%d. inn=%d, jnn=%d (dij=%.2e) of %d pairs. Intersection %s",
          k,kmed.i_meds[k],nmem_clst_vec[k],ij_nn_pairs[k][0],ij_nn_pairs[k][1],dpairs_ckrn[k][1],i_n_pairs_ckrn[k][1],
          (f_reduce_success[k])?("FOUND: "):("NOT found\n"));
        if (f_reduce_success[k])
        {
          if (i_n_pairs_ckrn[k][0]) printf("i_cok=%d, j_cok=%d (d=%.2e, %d). ",
              ij_pairs_ckrn[k][0],ij_pairs_ckrn[k][1],dpairs_ckrn[k][0],i_n_pairs_ckrn[k][0]);
          printf("nuld_i=%d, nuld_j=%d -> nuld_ij=%d \n",
            ckrn_spcs[ij_pairs_ckrn[k][0]]->nvec_use, ckrn_spcs[ij_pairs_ckrn[k][1]]->nvec_use,nuld_ckrn[k]);
        }
      }

      if (nred_succ)
        nset = cokern_.collapse_cokernals(msr_,gen_+1,
          f_reduce_success,nuld_ckrn,ij_pairs_ckrn[0],svec_ckrns[0],Vmat_ckrns[0][0],isat_ckrn[0]);
