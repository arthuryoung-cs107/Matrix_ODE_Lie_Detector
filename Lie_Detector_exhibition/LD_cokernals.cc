...
int nullspace_ckrn_policy::iterate_nullspace_cokernals(cokernal_bundle &cokern_, k_medoids_package &kmed_, bool verbose_)
{
  const int vlen_full = msr.vlen;
  int &nset = cokern_.nset,
      &kSC = cokern_.kSC,
      &nred_succ = cokern_.nred_succ,
      &generation = cokern_.generation,
      nmem_clst_vec[kSC],
      imem_clst_vec[nset],
      *imem_clst_mat[kSC];
  ...
  if (nred_succ)
    nset = cokern_.collapse_cokernals(msr,
      f_reduce_success,nuld_ckrn,ij_pairs_ckrn[0],svec_ckrns[0],Vmat_ckrns[0][0],isat_ckrn[0],wdistance,verbose_);

  return nred_succ;
}

int cokernal_bundle::collapse_cokernals(LD_vspace_measure &msr_, bool *frdc_succ_, int *nsat_ckrn_, int *ij_prs_, double *w_ckrn_, double *vvec_ckrn_, int *isat_ckrn_,bool wdistance_,bool verbose_)
{
  ...
  for (size_t i = 0, irdc = 0, i_ij = 0, iw = 0, iv = 0; irdc < nred_succ; irdc += (int)(frdc_succ_[i++]), i_ij+=2, iw+=vlen, iv+=vl2)
    if (frdc_succ_[i])
    {
      ...
      if ( ckrn_i_old->gen_spawn ) // if NOT an original kernal, can be overwritten
        ckrn_spcs[i_i] = new cokernal_space(generation+1,ckrn_i_old,ckrn_j_old); // spawn new cokernal space by overwriting ckrn_i and its workspace
      else // if an original kernal, extra workspace must be overwritten or allocated
      {
        ...
        ckrn_spcs[i_i] = new cokernal_space(generation+1,ckrn_wkspcs_xtra[i_check],ckrn_i_old,ckrn_j_old);
      }
  ...
  for (size_t i = 0, ickrn_new = 0; ickrn_new < nset_new; i++)
    if (ckrn_spcs[i] != NULL)
    {
      ...
      Wmat[ickrn_new] = ckrn_spcs[i]->wvec;
      vspcs[ickrn_new] = ckrn_wkspcs[ckrn_spcs[i]->iwkspc]->vspc;
  ...
  // if (wdistance_) msr_.init_distances(vspcs,Wmat,nset_new,verbose_); // recompute (expensive), but definitely correct weighted distances
  // else msr_.init_distances(vspcs,nset_new,verbose_); // recompute (expensive), but definitely correct distances

  if (wdistance_) msr_.update_distances(nred_succ,ij_prnts[0],vspcs,Wmat,verbose_); // update weighted distances
  else msr_.update_distances(nred_succ,ij_prnts[0],vspcs,verbose_); // update distances
