classdef LDsol
    ...
    methods (Static)
        function [sol,f_s0_out,dxf_s0_out,vth_out,lam_out,Jtu_s0_out,JtuN_s0_out] = initialize_origin(s0_,fspc_,WR_,iP_,ndep0_)
            nvar_ = length(s0_(:));
            Plen_b = size(fspc_.Pmat(:,iP_),2);

            lam_out = adlam( fspc_, s0_ );
            vx_W = WR_(1:Plen_b,:)' * reshape( lam_out.lrow_vals(iP_) , [], 1 );
            vth_out = WR_ * (vx_W./sum(vx_W.^2,1));
            ...
            sol = LDsol(s0_);
            sol.lam_v = lam_out;
            sol.s = [ s0_ ; f_s0_out((end-ndep0_+1):end) ];
            sol.dNp1xu = dxf_s0_out( (end-ndep0_+1):end );

            sol.WR = WR_;
            sol.iPv = iP_;
            sol.vth = vth_out;
            sol.JtuN = JtuN_s0_out;

            sol.tN = [1 ; sol.s((ndep0_+2):end) ; sol.dNp1xu(:)];
            sol.JF = [ sol.JtuN((end-ndep0_+1):end,1:nvar_,1) , -eye(ndep0_) ];
        ...
        function mod_out = model_solspace(Sobs_,dat_,fmap_)
            ...
            function [Asvd_out,U_,V_] = Asvd_package(A_);
                dim_scl = max(size(A_));
                if (nargout==1)
                    [~,S_,V_] = svd(A_,'econ');
                elseif (nargout==2)
                ...
                Asvd_out = struct( ...
                    ...
                    's', s_, ...
                    'V', V_, ...
                    'D',[ s_(:)' , zeros(1, size(V_,2)-length(s_) ) ] .* V_, ... % rowspace (domain) Y approx. basis
                    'W',[(s_(end)./s_(:))',ones(1,size(V_,2)-length(s_))].*V_ ... % nullspace (kernal) K approx. basis
                );
            end
            ...
            for iobs = 1:nobs
                ...
                l_imm_i = immerse_l_Lamspc(lv_RN1_i,ndep_N1);
                ...
                LamN_T_tns_RN1(:,(nvar_N1+1):(nvar_N1+ndep_N1),iobs) = ...
                    [ -lRN1_i.lkx(:,:,1) , immerse_l_Lamspc(dxl_RN1_i,ndep_N1) ]';
                iiLam0 = nvar + (1:ndep);
...
fprintf('(LDsol::model_solspace) Decomposed %d R + DprN matrices in %.2f seconds: %dx%d (r=%d,k=%d) -> %dx%d (r=%d,k=%d,o0=%d,oN=%d,o=%d) \n', ...
            ...
            tau_uN_RN1_net = comp_tau_uN_RN1( vth_RN1_net , inds_P_RN1 ); % ndep_N1 by 2 by nobs
            tauN1_S_mat = [ ones(1, nobs) ; reshape(tau_uN_RN1_net,ndim_N1-1,nobs) ];
...
fprintf('(LDsol::model_solspace) Decomposed %d G+DprN matrices in %.2f seconds: %dx%d (r=%d,k=%d) -> %dx%d (r=%d,k=%d,o0=%d,oN=%d,o=%d) \n', ...
            ...
            % RGsvd_N1_net = Asvd_package([ Rmat_N1_net ; Gmat_N1 ]);

            % W_R_sO = Rsvd_N1_net.W;
            W_R_sO = RGsvd_N1_net.W;
            ...
            [sol_O,f_O0,dxf_O0,vth_sO,lamRN1_sO,Jtu_sO,JtuN_sO] = LDsol.initialize_origin( ...
                s_O0, fspace_RN1, W_R_sO, inds_P_RN1, ndep ...
            );
            sol_O.lamN1 = lamN1_sO;
            sol_O.lamRN1 = lamRN1_sO;
            % % pass s_O0 to tvf model, get s_N, N+1'th derivative, and Jacobian at the origin
            % [f_O0,dxf_O0,vth_sO,lamRN1_sO,Jtu_sO,JtuN_sO] = comp_f_s0(s_O0,fspace_RN1,W_R_sO,inds_P_RN1);
            ...
            function [flow_out,Gc_svd] = verify_flow_transformation(iPv_,tO_,JtuN_sO_,lam_sO_)
                ...
                % VdNxuspc_sO is ndep x C matrix, columns are vfield coeffs of dNxu in jet space
                VdNxuspc_image = @(thmat_,thtns_) ...
                    reshape( pagemtimes( thtns_((end-ndep+1):end,:,:) , dxl_b_sO(:) ) , [ndep size(thtns_,3)] ) ...
                    - l1x_b_sO((end-ndep+1):end,:,1) * thmat_(1:Plen_v,:) ;
                % VdN1xuspc_sO is Q x C matrix, columns are vfield coeffs of dNxu in jet space
                VdN1xuspc_image = @(thmat_,thtns_) ...
                    reshape( pagemtimes( thtns_(2:end,:,:) , dxl_b_sO(:) ) , [ndep_N1 size(thtns_,3)] ) ...
                    - l1x_b_sO(:,:,1) * thmat_(1:Plen_v,:) ;
                % B x C matrix, columns are vfield coeffs, span tangent space of SN at sNO, lie algebra at origin
                VNspc_image = @(thmat_,thtns_) [ V0spc_image(thtns_) ; VdNxuspc_image(thmat_,thtns_) ];
                % BN1 x C matrix, columns are vfield coeffs, span tangent space of SN at sNO, lie algebra at origin
                VN1spc_image = @(thmat_,thtns_) [ V0spc_image(thtns_) ; VdN1xuspc_image(thmat_,thtns_) ];
                ...
                compute_sO_N1_Tspc_image = @(thmat_) VN1spc_image( ...
                    thmat_ , permute(reshape(thmat_,Plen_v,nvar_N1,size(thmat_,2)),[2 1 3]) );

                tN_O_unit = tO_ / norm(tO_);
                tuN1_O_mat = [ tO_(1+(1:ndep_N1)) ; tO_((end-ndep_N1+1):end) ];
                tN1_O = [ 1 ; tuN1_O_mat(:) ];
                tN1_O_unit = tN1_O / norm(tN1_O);
                Jt0_sO = [ zeros(1,ndim) ; [ JtuN_sO_(:,1:nvar_N1,1) , zeros((nvar_N1-1),ndep) ] ]; % ndep_N1 x nvar_N1
                Jf_sO = Jt0_sO( (end-ndep+1):end,1:nvar_N1 ); % ndep x nvar_N1, grad of N'th derivatives at sO
                JF_sO = [ Jf_sO , -eye(ndep) ]; % ndep x ndim, JF = ( Jf , -I )
                [JF_sO_svd,U_JF_sO] =  Asvd_package(JF_sO');
                JF_sO_svd.U = U_JF_sO;
                P_JF_sO = eye(ndim) - U_JF_sO*U_JF_sO';
                % DprN condition baked in, alongside infinitesimal criterion
                JF_N1_sO = [ ...
                    [ Jf_sO zeros(ndep_N1-ndep) -eye(ndep)] ; ...
                    [ zeros(ndep_N1-ndep,nvar) eye(ndep_N1-ndep) -eye(ndep_N1-ndep) zeros(ndep_N1-ndep,ndep) ] ...
                ];
                ...
                for idep = 1:ndep_N1
                    LamN1_v_tns_sO(:,idep+1,idep+1) = lv_b_sO(:);
                    LamN1_v_tns_sO(:,idep+1,idep+nvar_N1) = dxl_b_sO(:);
                    LamN1_v_tns_sO(:,1,idep+nvar_N1) = -l1x_b_sO(idep,:)';
                end
                LamN1_v_mat_sO = reshape(LamN1_v_tns_sO,[ntheta_v ndim_N1])';
                G_N1_sO = JF_N1_sO*LamN1_v_mat_sO;

                Bmat_t0_sO = ...
                    immerse_l_Lamspc(dxl_b_sO,nvar_N1) ...
                    - [ zeros(1,nvar_N1) ;
                        JtuN_sO_(:,1:nvar_N1,1) ] * LamN1_v_mat_sO(1:nvar_N1,:);
                Bmat_tdxu_sO = ...
                [ permute(pagemtimes(tN1_O',permute(-Jl1xv_b_sO, [3 2 1])),[3 2 1]) , ...
                  immerse_l_Lamspc(tN1_O'*Jdxlv_b_sO,ndep_N1) ] ...
                - (JtuN_sO_(:,:,2) * LamN1_v_mat_sO ) ;
                B_tN1_sO = [Bmat_t0_sO ; Bmat_tdxu_sO];
                ...
                if (kor>1)
                    ...
                    iiLam1 = iiLam0-nvar+nvar_N1;
                    for k = 2:kor
                        DprN_tns_sO(:,:,k-1) = LamN1_v_mat_sO(iiLam1,:)' - LamN1_v_mat_sO(iiLam0,:)';
                        iiLam0 = iiLam0 + ndep;
                        iiLam1 = iiLam1 + ndep;
                    end
                ...
                LamN_T_v1_ttns = LamN_T_vN_ttns(:,:,iN1_2_N,:); % Plen x nvar_N1 x ndim x nobs
                ...
                function [Vspc_Th_i,VNspc_Th_i0,VN1spc_Th_i0] = compute_transversal_Tspace(Th_,VNi_unit_sO_)
                ...
                VN1_spc_sO = zeros(ndim_N1,nvar_N1);
                ...
                [mu_N1_sO_svd,U_mu_N1_sO] = Asvd_package(Tspc_N1v_sO_mat0); mu_N1_sO_svd.U = U_mu_N1_sO;
                %% rank principle vector fields by their tangent vector component in the gradient space of S
                [UJFT_Umu_srt,isrt_UJFT_Umu] = sort( sum((U_JF_N1_sO' * U_mu_N1_sO).^2,1) );

                [TTspc_svd_0,U_TTspc_0] = Asvd_package(TTspc_0); TTspc_svd_0.U = U_TTspc_0;
                dimY0 = TTspc_svd_0.r; % should always be ndep*kor = ndep_N1
                % gspc_sO_svd_0 = Asvd_package( U_JF_N1_sO'*Tspc_N1v_sO_mat0*TTspc_svd_0.V(:,1:dimY0) );
                gspc_sO_svd_0 = Asvd_package( JF_N1_sO*Tspc_N1v_sO_mat0*TTspc_svd_0.V(:,1:dimY0) );
                ...
                Gn_bse.mu_N1_sO_svd = mu_N1_sO_svd;
                Gn_bse.UJFT_Umu_srt = UJFT_Umu_srt;
                Gn_bse.isrt_UJFT_Umu = isrt_UJFT_Umu;
                ...
                for ivec = 1:ndep_N1
                    ...
                    %% identify subspace of candidate vector fields transversal to current basis at origin
                    [TTspc_i,Tspc_Nv_sO_tns(:,:,ivec),Tspc_N1v_sO_mat0_i] = ...
                        compute_transversal_Tspace(Theta_N_i,VN1_spc_unit_sO(:,1:ivec));
