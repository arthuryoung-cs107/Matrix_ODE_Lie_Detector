classdef apv_plots
    ...
    methods (Static)
        ...
        function [plt,plt_jspc] = plot_LDsol_model_summary(p_,mod_,d_)
            ...
            function plot_labelled_svds(axi_,svds_,lbls_,mrkr_,leg_loc_,nrm_flg_)
                if (nargin==4)
                    nrm_flg = false;
                else
                    nrm_flg = nrm_flg_;
            ...
            % A_G_sO = nan(ndim,length(mod_.flow_pckg.gspc_sO_svds(:)));
            % for i = 1:size(A_G_sO,2)
            %     A_G_sO(:,i) = sum(mod_.flow_pckg.gspc_sO_svds(i).U .*  mod_.flow_pckg.gspc_sO_svds(i).s' , 2);
            ...
            Aspcs = { ...
                A_G_sO, A_G_sO_u, Aspc.cmat, Aspc.lspc, Aspc.mspc ; ...
            };
            ...
            % Vspc_svd_i = mod_.flow_pckg.gspc_sO_svds(1);
            % Vspc_O = M_sO_basis.Vspc_sO;
            ...
            for i = 1:ndep
                ...
                for iA = 1:size(Aspcs,1)
                    for iiA = 1:size( Aspcs{iA,1} , 2 )
                        plot_tvector( ...
                            axs_mat(i,1),xO,uO(i,1),Aspcs{iA,1}(1,iiA),Aspcs{iA,2}(i,1,iiA),Aspcs{iA,3}(iiA,:),Aspcs{iA,4},Aspcs{iA,5});
                    end
                end
                plot2D( ...
                ...
                for k = 2:(kor+1)
                    ...
                    for iA = 1:size(Aspcs,1)
                        for iiA = 1:size( Aspcs{iA,1} , 2 )
                            plot_tvector( ...
                                axs_mat(i,k),xO,uO(i,k),Aspcs{iA,1}(1,iiA),Aspcs{iA,2}(i,k,iiA),Aspcs{iA,3}(iiA,:),Aspcs{iA,4},Aspcs{iA,5});
                        end
                    end
                    plot2D( ...
                    ...
                    for iV = 1:size(Vspcs,1)
                        for iiV = 1:length( Vspcs{iV,1} )
                            plot_tvector( ...
                                axs_mat(i,k),xO,uO(i,k),Vspcs{iV,1}(iiV),Vspcs{iV,2}(i,k,iiV),Vspcs{iV,3}(iiV,:),Vspcs{iV,4},Vspcs{iV,5}, ...
                                ['$ ( ' Vspcs{iV,6}(iiV) '_x,' Vspcs{iV,6}(iiV) '_{' dkxui_str(k-1,i) '} ) |_{s_O} $'] ...
                            );
                        end
                    end
                end
            ...
            svds_i = { ...
                mod_.Jl_N1_svd , '$J_{\lambda} |_{ \{ s_j \} } ', 's'; ...
                mod_.H_N1_svd , '$H^{(0)}_{\textrm{tvf}}', 's'; ...
                mod_.DprN_svd, '$D_{\textrm{pr}}', 's'; ...
                mod_.Rsvd_N1_net, '$R^1_{\mathrm{net}}', 'd'; ...
                mod_.Bsvd_t0_N1,  '$B^{(0)}', 'x'; ...
            ...
            for isvd = 1:size(svds_i,1)
                ...
                markrs{isvd} = svds_i{isvd,3};
            end
            plot_labelled_svds(axi,svds,labels,markrs,'EastOutside',true);

            axi = axs_mat(1,2);
            title(axi, ['SVDs over base/jet space (at the origin) '], ...
                'Interpreter','Latex','FontSize',12 );
            svds_i = { ...
                mod_.Jl_N1_svd.gEta_tvf_YJl_sO_svd, '$\nabla^{(0)} ( \eta_i )_{i=1}^{\rho} ', 's'; ...
                mod_.Mcom_sO_basis , '$\Lambda^{(1)}_{O} W_{G_c}', 's'; ...
                mod_.Mnet_sO_basis , '$\Lambda^{(1)}_{O} W_{G_n}', 's'; ...
                mod_.Ncom_sO_basis , '$\Lambda^{(1)}_{O} W_{G_c} D_{N_c}', 's'; ...
                mod_.Nnet_sO_basis , '$\Lambda^{(1)}_{O} W_{G_n} D_{N_n}', 's'; ...
                mod_.flow_pckg.JF_sO_svd, '$JF|_{s_O} ', 'v'; ...
                mod_.flow_pckg.JF_N1_sO_svd, '$JF^{(\mathbf 1)}|_{s_O}', '^'; ...
                mod_.flow_pckg.Gn_bse.mu_N1_sO_svd,  '$\mu^{(\mathbf 1)}_{O}', 'd'; ...
                mod_.flow_pckg.Gn_bse.TTspc_svd_0 , '$T_0 |_{s_O}', 's'; ...
                mod_.flow_pckg.Gn_bse.gspc_sO_svd_0 , '$g_0 |_{s_O}', 's'; ...
            };

            nsvd_i = size(svds_i,1);
            Tsvds_i = mod_.flow_pckg.TTspc_svds;
            ...
            nsvd_i = size(svds_i,1);
            gsvds_i = mod_.flow_pckg.gspc_sO_svds;
            for isvd = 1:length(gsvds_i(:))
                svds_i{isvd+nsvd_i,1} = gsvds_i(isvd);
