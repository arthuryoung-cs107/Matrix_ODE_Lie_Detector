classdef LD_denoise_plots < LD_plots
    ...
    methods (Static)
        function [plts_out,dnse_data] = plot_denoising_summary(plts_,spc_,Ssets_,ode_,icrv_)
            ...
            for j = 1:ncrv
                res_nse_ref = res_nse_ref + sum( (pts_nse_cell{j}(2:end,:) - pts_ref_cell{j}(2:end,:)).^2 ,2);
                res_nse_sys = res_nse_sys + sum(( pts_nse_cell{j}((end-ndep+1):end,:) - ode_.dnxu(pts_nse_cell{j}(1:(ndim-ndep),:)) ).^2, 2);
            end
            ...
            for i = 1:len_iwrite_full
                ...
                for j = 1:ncrv
                    res_tru(:,i) = res_tru(:,i) + ...
                        sum( (Sdnse_cell_i{j}(2:end,:) - pts_ref_cell{j}(2:end,:)).^2 ,2);

                    res_sys(i) = res_sys(i) + ...
                        sum(( Sdnse_cell_i{j}((end-ndep+1):end,:) - ode_.dnxu(Sdnse_cell_i{j}(1:(ndim-ndep),:)) ).^2, 2);
                end
            end
